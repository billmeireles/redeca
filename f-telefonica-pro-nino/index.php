...
require_once('setup_constants.php');

$path = strrev($_SERVER['PHP_SELF']);
$path = substr($path, strpos($path, "/"));
$path = strrev($path);

if((!in_array(DIRECTORY_INSTALL, $arr_request_uri) && is_dir(DIRECTORY_INSTALL)) || is_dir(DIRECTORY_INSTALL)){
	if(file_exists(DIRECTORY_INSTALL.DIRECTORY_SEPARATOR.FILE_INSTALLED))
		header('Location: http://' . $_SERVER['HTTP_HOST'] . $path . 'install/finish.php');
	else	
		header('Location: http://' . $_SERVER['HTTP_HOST'] . $path . 'install/index.php');
}else {

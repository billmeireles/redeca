...
class HistoryController extends BasicController
{
	...
	/**
	 * Exibe o histÃ³rico de alterações de acordo com o mÃ³dulo atual
	 */
	...
	function viewAction()
	{
		...
		if($hist && count($hist) > 0)
		{
			...
			elseif($hist->{PCH_TABLE_NAME} == TBL_HEALTH)
			{
				$this->view->careUnit = HealthBusiness::loadCareUnityById($this->view->healthByPerson->id_entity);;
			}
			elseif($hist->{PCH_TABLE_NAME} == TBL_VACCINATION)
			{
				$this->view->vaccine = HealthBusiness::loadVaccineById($this->view->vaccinationByPerson->id_vaccine);
			}

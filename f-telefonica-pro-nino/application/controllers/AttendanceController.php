...
class AttendanceController extends BasicController {
	...
	function attendanceAction() {
		...
		$this->view->assistances = AssistanceBusiness::buildAssistanceSummaryAttendance ( $assistances );
	...
	function groupAction() {
		...
		$class = ClassBusiness::load ( $data ['id_class'] );
		
		if ($data ['id_status'] == 1) {
			//insere uma pessoa de determinada turma
			ClassBusiness::saveClassAssistance ( $data, $class, "Inserção na Turma" );
			$this->_redirect ( ATTENDANCE_CONTROLLER . '/' . DEFAULT_SUCCESS_ACTION . '/' . $form->personId () . '/' . $form->getPersonId () );
		}
		
		if ($data ['id_status'] == 2) {
			//insere uma pessoa na fila de espera de determinada turma
			ClassBusiness::saveClassAssistance ( $data, $class, "Inserção na Fila de Espera" );
			$this->_redirect ( ATTENDANCE_CONTROLLER . '/' . DEFAULT_SUCCESS_ACTION . '/' . $form->personId () . '/' . $form->getPersonId () );
		}
	}
	...
	function setAssistanceType($programId) {
		try {
			...
			if ($assistType === NULL) {
				...
				throw new Zend_Exception ( 'Invalid config parameters:' . ' assistance.classification.general => ' . $config->assistance->classification->general . ' assistance.classification.group => ' . $config->assistance->classification->group . ' assistance.classification.especial => ' . $config->assistance->classification->especial );
			}
	...
	function confirmAction() {
		...
		$this->view->assistance = AssistanceBusiness::buildAssistance ( $assistance, $form->getClassId (), null );
	}

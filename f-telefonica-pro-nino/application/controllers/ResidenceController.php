...
class ResidenceController extends BasicController
{
	...
	function telephoneAction()
	{
		...
		// adiciona novo ID de telefone no formulário
		$idd = self::telephoneManager($telephone);
		$id = $this->view->form->getIdPhone();
		$id[] = $idd;
		$this->view->form->setIdPhone($id);
	}
	...
	function telephoneManager($telephone)
	{
		...
		{			
			if(!is_null($telephone))
			{
				...
				// Carrega em um Array todos os telefones da pessoa que estão no banco
				foreach ( $telephone as $tel ) {
					$row = TelephoneBusiness::loadById($tel[TNB_ID_TELEPHONE_NUMBER]);
					if (! is_null($row))
						$telBD[] = $row->number;
				}
				...
				foreach($telephone as $tel)
				{
					...
					$flag = false;
					// compara telefone inserido com telefones no banco
					foreach ($telBD as $number) {
						if ($number == $tel[TNB_NUMBER]) {
							$flag = true;
							break;
						}
					}

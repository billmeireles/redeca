...
class AuthController extends BasicController {
	...
	/**
	 * Exibe o formulário de autenticação e destroi qualquer sessão do sistema aberta
	 * 
	 */
	...
	function loginAction() {
		...
		if (UserLogged::isManager () || UserLogged::isCoordinator ())
			$this->_redirect ( ENTITY_CONTROLLER );
		
		if (UserLogged::isAdministrator ()) {
			$config = new Zend_Config_Ini ( 'application/config.ini' );
			
			if ($config->sendForm != null) {
				$this->_redirect ( ENTITY_CONTROLLER );
			} else {
				$this->_redirect ( ENTITY_CONTROLLER.'/createform' );
			}
		}

...
class EntityController extends BasicController {
	...
	function editAction() {
		...
		UserValidator::validateUserEqualCpf ( $userFrm->getId (), $userFrm->getCpf (), $errorMessages );
		// mudança na validação
		UserValidator::validateUserId ( $userFrm, $errorMessages );
		if ($userFrm->getFlagPassword () && strlen ( $userFrm->getPassword () ) <= 0)
			UserValidator::validateUserPassword ( $userFrm, $errorMessages );
		if (! $errorMessages [pass])
			print_r ( $errorMessages [pass] );
	...
	public function sendFormAction() {
		...
		if ($this->getRequest ()->isPost ()) {
			$form = array ();
			//recebe os dados do formulário preenchido
			$form ['name'] = $this->_request->getPost ( 'name' );
			$form ['email'] = $this->_request->getPost ( 'email' );
			$form ['phone'] = $this->_request->getPost ( 'phone' );
			$form ['city'] = $this->_request->getPost ( 'city' );
		}
		//recebe o sinal de que o formulário foi fechado
		$flag = $this->_request->getPost ( 'hidden' );
		...
		$errorMessages = EntityValidator::validateForm ( $form );
		
		//se o formulário for fechado persiste apena o ip do cliente
		if ($flag == 'close') {
			echo $this->abreurl ( $config->joomla->host . "&ip=" . $ip . "" );
			$this->_redirect ( ENTITY_CONTROLLER );
		} //senão verifica se não há error no formulário 
		else if (sizeof ( $errorMessages ) > 0) {
			$this->view->param = $form;
			$this->view->error = $errorMessages;
		
		} else {
			echo $this->abreurl ( $config->joomla->host . "&name=" . str_replace ( ' ', '%20', $form ['name'] ) . "&email=" . $form ['email'] . "&phone=" . $form ['phone'] . "&city=" . str_replace ( ' ', '%20', $form ['city'] ) . "&ip=" . $ip . "" );
			$adminFlag = fopen ( "application/config.ini", "a" );
			fwrite ( $adminFlag, "\n\n[sendForm]Administrador enviou o formulário em " . date ( "d/m/Y" ) );
			fclose ( $adminFlag );
			$this->_redirect ( ENTITY_CONTROLLER . '/' . DEFAULT_SEND_SUCCESS_ACTION );
		}
	}
	...
	function abreurl($url) {
		...
		curl_setopt ( $ch, CURLOPT_HEADER, 1 ); // Retorna o cabeçalho da página
		curl_setopt ( $ch, CURLOPT_RETURNTRANSFER, 1 ); // Retorna o HTML

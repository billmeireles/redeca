...
class BasicParser
{
	...
	public static function parseSingleRegister($folder, $fileNames, $numberOfLines=0, &$flag=NULL)
	{
		...
		Zend_Loader::loadClass('AddressParser');
		Zend_Loader::loadClass('BuildEntityParser');
		Zend_Loader::loadClass('CivilCertificateParser');
		Zend_Loader::loadClass('ConsanguineParser');
		Zend_Loader::loadClass('CtpsParser');
		Zend_Loader::loadClass('DeficiencyParser');
		Zend_Loader::loadClass('DocumentParser');
		Zend_Loader::loadClass('EmploymentParser');
		Zend_Loader::loadClass('FamilyIdParser');
		Zend_Loader::loadClass('FamilyRelationParser');
		Zend_Loader::loadClass('FamilyResidenceParser');
		Zend_Loader::loadClass('IncomeParser');
		Zend_Loader::loadClass('LevelInstructionParser');
		Zend_Loader::loadClass('RegistrationParser');
		Zend_Loader::loadClass('RepresentativeParser');
		Zend_Loader::loadClass('ResidenceParser');
		Zend_Loader::loadClass('SchoolParser');
		Zend_Loader::loadClass('SocialProgramParser');
		Zend_Loader::loadClass('PersonParser');
		...
		if(!empty($fileNames) && $fileNames !== NULL)
		{
			...
			foreach($fileNames as $currentFile)
			{
				...
				if($currentFile !== NULL && !is_dir($folder.'/'.$currentFile))
				{
					...
					foreach ($lines as $line_num => $line){
						...
						if(!empty($data[$i])){
							...
							if(!empty($data[$i]['_201_NomPessoa']))
							{
								...
								$familyResidence[FRS_LIVE_SINCE] 				= self::validateParam($data[$i]['_103_DtaPesquisaDomic'], BasicParser::is_numeric(), 'intval');
								...
								$residence[RES_NUMBER]						    = self::validateParam($data[$i]['_204_NumResidenciaDomic'], BasicParser::is_numeric(), 'intval');
								$residence[RES_COMPLEMENT] 						= self::validateParam($data[$i]['_205_NomComplResidenciaDomic'], BasicParser::is_string(), 'strval');
								$residence[RES_REFERENCE_POINT]					= null;
								$residence[RES_ID_STATUS]		 				= self::validateParam($data[$i]['_213_SitDomicilioDomic'], BasicParser::is_numeric(), 'intval');
								$residence[RES_ID_MORADA]		 				= self::validateParam($data[$i]['_214_TipDomicilioDomic'], BasicParser::is_numeric(), 'intval');
								$residence[RES_ACCOMMODATION]		 			= self::validateParam($data[$i]['_215_NumComodosDomic'], BasicParser::is_numeric(), 'intval');
								$residence[RES_ID_BUILDING]		 				= self::validateParam($data[$i]['_216_TipConstrucaoDomic'], BasicParser::is_numeric(), 'intval');
								$residence[RES_ID_SUPPLY]						= self::validateParam($data[$i]['_217_TipAbastecimentoAguaDomic'], BasicParser::is_numeric(), 'intval');
								$residence[RES_ID_WATER]						= self::validateParam($data[$i]['_218_TipTratamentoAguaDomic'], BasicParser::is_numeric(), 'intval');
								$residence[RES_ID_ILLUMINATION]					= self::validateParam($data[$i]['_219_TipIluminacaoAguaDomic'], BasicParser::is_numeric(), 'intval');
								$residence[RES_ID_SANITARY]						= self::validateParam($data[$i]['_220_TipEscoamentoSanitarioDomic'], BasicParser::is_numeric(), 'intval');
								$residence[RES_ID_TRASH]						= self::validateParam($data[$i]['_221_TipDestinoLixoDomic'], BasicParser::is_numeric(), 'intval');
								$residence[RES_ID_LOCALITY] 					= self::validateParam($data[$i]['_211_TipLocalDomic'], BasicParser::is_numeric(), 'intval');
								...
								$address										= array();
								$address[ADR_ZIP_CODE]							= self::validateParam($data[$i]['_201_CepResidenciaDomic'], BasicParser::is_string(), 'strval');
								$address[ADR_ADDRESS]							= self::validateParam($data[$i]['_203_NomLogradouroDomic'], BasicParser::is_string(), 'strval');
							...
							if($data[$i][Constants::ARRAY_TYPE] == Constants::ROWTYPE_PERSON)
							{
								...
								$person[PRS_BIRTH_DATE] 							= self::validateParam($data[$i]['_202_DtaNascPessoa'], BasicParser::is_numeric());
								...
								$document[DOC_RG_EMISSION_DATE]						= self::validateParam($data[$i]['_226_DtaEmissaoIdentPessoa'], BasicParser::is_numeric());
								...
								$ctps[CTS_EMISSION]									= self::validateParam($data[$i]['_231_DtaEmissaoCartTrabPessoa'], BasicParser::is_numeric());
								...
								$civilCertificate[CCF_EMISSION_DATE]				= self::validateParam($data[$i]['_221_DtaEmissaoCertidPessoa'], BasicParser::is_numeric());
								...
								if($employment[EMP_COMPANY_NAME] != null){
									...
									$employment[EMP_START_DATE]						= self::validateParam($data[$i]['_245_DtaAdmisEmpresaPessoa'], BasicParser::is_numeric());
								...
								if($haveSop == true){
									$socialProgram									= array();
									$registerDate									= array();
									$registerDate[1]								= self::validateParam($data[$i]['_270_DtaIncPetiPessoa'], BasicParser::is_numeric());
									$registerDate[2]								= null;
									$registerDate[3]								= self::validateParam($data[$i]['_270_DtaIncAgjPessoa'], BasicParser::is_numeric());
									$registerDate[4]								= null;
									$registerDate[5]								= null;
									$registerDate[6]								= self::validateParam($data[$i]['_270_DtaIncProgerPessoa'], BasicParser::is_numeric());
									$registerDate[7]								= null;
									$registerDate[8]								= null;
					...
					Logger::loggerImport('A importacao foi concluida com sucesso.');
					echo "A persistencia foi finalizada.";
				}
			}
		}
	}

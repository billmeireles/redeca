 ...
 * Author                                           Date                               History 
 * -----------------------------------------        ------------                       ------------------ 
 * Fabricio Meireles Monteiro  - S2it		   		19/03/2008	                       Create file 
 * 
 */
...
abstract class BenefitValidator extends BasicValidator
{
	...
	{	
		...
		{	
			...
			if(!$validatorRequired->isValid($stringDate))
			{
				$errorMessages[BenefitForm::dateBenefit()."_".$objectBenefit[SPG_ID_SOCIAL_PROGRAM]][] = $validatorRequired->getMessages();
			}
			else if((strlen($stringDate) > 0 ) && !$validatorDate->isValid($objectBenefit[SPG_REGISTER_DATE.$objectBenefit[SPG_ID_SOCIAL_PROGRAM]]))
			{	

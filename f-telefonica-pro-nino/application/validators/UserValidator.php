...
abstract class UserValidator extends BasicValidator
{
	...
	public static function validateUserEqualCpf($id, $cpf, &$errorMessages = null)
	{
		Zend_Loader::loadClass('UserBusiness');
		$row = UserBusiness::loadUserByCpf($cpf);	
		if(count($row) > 0)
		{
			if($row->{id_user} != $id)
			{
	...
	{	
		...
		{
			$stringLenght = parent::validatorStringLength(3, 90);
			if(strlen($frm->getName()) > 90)
			{
				$userName = Utils::abbreviate($frm->getName(), 91);
		...
		{
			$stringLenght = parent::validatorStringLength(4, 30);
			
			if(strlen($frm->getLogin()) > 30)
			{
				$login = Utils::abbreviate($frm->getLogin(), 31);
	...
	public static function validateUserAdd(UserForm &$frm)
	{
		...
		UserValidator::validateUserEqualCpf($frm->getId(), $frm->getCpf(), $errorMessages);

 ...
 * Author                                           Date                               History 
 * -----------------------------------------        ------------                       ------------------ 
 * Fabricio Meireles Monteiro  - S2it		   		25/02/2008	                       Create file 
 * 
 */
...
abstract class EntityValidator extends BasicValidator {
	...
	public static function validateEntityName(EntityForm &$frm, &$errorMessages = null) {
		...
		if (! $notEmpty->isValid ( $frm->getEntityName () )) {
			...
			$stringLenght = parent::validatorStringLength ( 3, 100 );
			
			if (strlen ( $frm->getEntityName () ) > 100) {
				$entityName = Utils::abbreviate ( $frm->getEntityName (), 101 );
	...
	/*
	 * Valida formulário para o Joomla
	 */
	public static function validateForm($form) {
		
		if (strlen ( $form ['name'] ) > 65) {
			$errorMessages ['name'] = "Campo Nome deve ter no máximo 65 caracteres";
		}
		if (strlen ( $form ['name'] ) == 0) {
			$errorMessages ['name'] = "Campo Nome é obrigatório";
		}
		else if (trim ( $form ['name'] ) == '') {
			$errorMessages ['name'] = "Campo Nome é obrigatório";
		}
		if (strlen ( $form ['email'] ) > 65) {
			$errorMessages ['email'] = "Campo Email deve ter no máximo 65 caracteres";
		}
		if (strlen ( $form ['email'] ) == 0) {
			$errorMessages ['email'] = "Campo Email é obrigatório";
		}
		else if (trim ( $form ['email'] ) == '') {
			$errorMessages ['email'] = "Campo Email é obrigatório";
		}
		if (strlen ( $form ['phone'] ) > 65) {
			$errorMessages ['phone'] = "Campo Telefone deve ter no máximo 65 caracteres";
		}
		if (strlen ( $form ['city'] ) > 65) {
			$errorMessages ['city'] = "Campo Cidade deve ter no máximo 65 caracteres";
		}

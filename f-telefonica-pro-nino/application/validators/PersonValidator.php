 ...
 * Author                                           Date                               History 
 * -----------------------------------------        ------------                       ------------------ 
 * Saulo Esteves Rodrigues  - S2it		   			10/01/2008	                       Create file 
 * 
 */
...
abstract class PersonValidator extends BasicValidator
{
	...
	*/
	public static function validateIdAddress(PersonForm $frm, &$errorMessages = null)
	{
		...
		/* Qualquer campo extra ('Número', 'Complemento', 'Referência' ou 'Moda desde') informado 
		 * sem o endereço deve ser barrado*/ 		
		$address = $notEmpty->isValid($frm->getAdrNumber());
		if(!$address)
			$address = $notEmpty->isValid($frm->getAdrComplement());
		if(!$address)
			$address = $notEmpty->isValid($frm->getAdrReference());
		if(!$address)
			$address = $notEmpty->isValid($frm->getLiveSince());

		if ($address && !$notEmpty->isValid($frm->getAdrIdAddress())) {
			$errorMessages[PersonForm::adr_idAddress()][]  = $notEmpty->getMessages();
		}else if($notEmpty->isValid($frm->getAdrIdAddress())) {
	...
	{
		$validator = parent::validatorStringLength(0, 500);
		
		if(strlen($frm->getTattoo()) > 500)
		{
			$tattoo = Utils::abbreviate($frm->getTattoo(), 31);
			$errorMessages[PersonForm::tattoo()][][] = parent::getValidatorResources()->text->long1.$tattoo.parent::getValidatorResources()->text->long2.'500'.parent::getValidatorResources()->text->long3;
		}

 ...
 * Author                                           Date                               History 
 * -----------------------------------------        ------------                       ------------------ 
 * Lucas dos Santos Borges Corrêa  - S2it		    02/05/2008	                       Create file 
 * 
 */
			...
			if(!empty($report))
			{
				...
				while($res = $report->fetch())
				{
					...
					if($newPage)
					{
						...
						$pdfPage->drawText('Page '.(count($pdf->pages)+1) , $LEFT_MARGIN, ($pageHeight - 25), $ENCONDING);
						$date = date('c',time());
						$pdfPage->drawText($date, 200 , ($pageHeight - 25), $ENCONDING);
						$title = $resource->view->labels->title->report;
						$pdfPage->drawText($title, 500, ($pageHeight - 25), $ENCONDING);
					}
					else
					{
						$yPos = $pageHeight - ($FIRST_LINE_POSITION+$i);
					...
					{							
						if($newPage)
						{
							...
							$columnHeader = self::getResourceText($resource,$k,'_');
							$pdfPage->drawText(Utils::abbreviate($columnHeader,$colWidth[$k] / $FACTOR), $widthCol, $yPos, $ENCONDING);
							$pdfPage->drawText(Utils::abbreviate($v,$colWidth[$k] / $FACTOR), $widthCol, $yPos - $i, $ENCONDING);
						}
						else
							$pdfPage->drawText(Utils::abbreviate($v,$colWidth[$k] / $FACTOR), $widthCol, $yPos, $ENCONDING);

 ...
 * Author                                           Date                               History 
 * -----------------------------------------        ------------                       ------------------ 
 * Fabricio Meireles Monteiro  - S2it		    	01/04/2008	                       Create file 
 * 
 */

require_once('BasicBusiness.php');
require_once('ClassHistory.php');
...
/**
	 ...
	 * @return ID
	 */
	public static function saveClassHistory($changeHistory, &$db=null)
	{
		$db->closeConnection ();
		$db = null;
		...
		try
		{	
			Zend_Loader::loadClass('ClassHistory');
			$obj = new ClassHistory();
			if($changeHistory != null)
			{
				$id = $obj->insert($changeHistory);
				Logger::loggerOperation('Registro inserido na tabela '. CH_ID_CLASS_HISTORY .' [id='.$id.']');
			}

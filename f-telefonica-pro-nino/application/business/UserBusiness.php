		...
		try
		{
			...
			{				
				if($user[F_ID_ROLE] == $config->user->role->coordinator)
				{
					...
					//verifica se a entidade selecionada já não possuia um coordenador
					if($user[F_ID_ENTITY] != null)
					{
						//remove o antigo coordenador da relação com a entidade selecionada
						UserBusiness::removeCoordinatorRelationshipWithEntityId($user[F_ID_ENTITY], $db);						
					}
				}
				else
				{
			...
			{
				//se o usuário administrado for coordenador, verificar
				//se houve troca da entidade do mesmo
				if($user[F_ID_ROLE] == $config->user->role->coordinator) 
				{
					//informações anteriores à nova persistência
					$userOriginal = UserBusiness::loadOneUser($user[F_ID_USER]);
					
					//verifica se houve troca da entidade
					if($userOriginal->{AUTH_ID_ENTITY_USER} != $user[F_ID_ENTITY])
					{
						//remove o antigo coordenador da relação com a entidade selecionada
						UserBusiness::removeCoordinatorRelationshipWithEntityId($user[F_ID_ENTITY], $db);						
					}
				...
				if(is_array($user[F_ID_PROFILE]))
				{	
					if(sizeof($user[F_ID_PROFILE]) == 1 && empty($user[F_ID_PROFILE][0])) {
						$profile = UserBusiness::loadUserProfile($user[F_ID_USER]);
						if(sizeof($profile) > 0){
							$userProfileObj = new UserProfile();
				...
				else if((sizeof($user[F_ID_PROFILE]) == 0))
				{
					$profile = UserBusiness::loadUserProfile($user[F_ID_USER]);
					if(sizeof($profile) > 0){
						$userProfileObj = new UserProfile();
...
//			return $db->fetchAll($select);
	...
	/**
	 * Verifica quem é o atual coordenador da entidade e remove o relacionamento
	 * 
	 */
	public static function removeCoordinatorRelationshipWithEntityId($entityId, $db)
	{
		//buscar qual o atual coordenador da entidade informada
		//e remove a relação com o antigo coordenador
		$ccordinator = UserBusiness::getCoordinator($entityId)->current();
		if($ccordinator != null)
		{
			$table = new User();
			$data = array();
			$data[AUTH_ID_ENTITY_USER] = null;
			$where = $table->getAdapter()->quoteInto(AUTH_ID_USER . " = ?", $ccordinator->{AUTH_ID_USER});
			$table->update($data, $where);
		}
	}
}
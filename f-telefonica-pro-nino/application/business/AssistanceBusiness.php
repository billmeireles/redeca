 ...
 * Author                                           Date                               History 
 * -----------------------------------------        ------------                       ------------------ 
 * Fabricio Meireles Monteiro  - S2it		    	28/03/2008	                       Create file 
 * 
 */
...
//	}
	...
	public static function buildAssistanceSummaryAttendance($assistances)
	{
		$data = NULL;
		if(!empty($assistances))
		{
			if(is_array($assistances))
			{
				foreach($assistances as $assistance)
				{
					$ast = self::buildAssistance($assistance, null, true);
					if($ast !== null)
						$data[] = $ast;
				}
			}
	 ...
	 */ 
	public static function buildAssistance($assistance, $idClass=null, $onlyPreview=false)
	{
		if(!empty($assistance))
		{			
			...
			if($assistType == Constants::GROUP)
			{
				...
				$isEmptyAssistance = true;
				
				if(!$onlyPreview && is_null($idClass)){
					$class = $groups->current();
					$isEmptyAssistance = false;	
				}
				else
				{	
					foreach($groups as $uniqueClass)
					{	
						if($onlyPreview)
						{
							$obj = new ClassAssistance();
							$row = $obj->find($uniqueClass->{CLS_ID_CLASS}, $assistance->{AST_ID_ASSISTANCE});
						...
						else if(!is_null($idClass))
						{
							if($idClass == $uniqueClass->{CLS_ID_CLASS})
							{
								$class = $uniqueClass;
								$isEmptyAssistance = false;
								break;

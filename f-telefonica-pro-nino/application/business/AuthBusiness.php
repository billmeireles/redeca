 ...
 * 
 * @copyright Fundaçãoo Telefônica - http://www.fundacaotelefonica.org.br 
 * 
...
class AuthBusiness extends BasicBusiness {
		...
		== md5($password)) {
			...
			if ($user-> {
				...
				!= $roleAdminId) {
					...
					$typeE = new Entity();
					$entity = $typeE->find($user-> {
						AUTH_ID_ENTITY_USER })->current();
					if ($entity-> {
						ENT_STATUS }) {
						$activeUser = true;
					} else
						if (($user-> {
							AUTH_ID_ROLE_USER }
					== $config->user->role->coordinator) && $user-> {
						AUTH_PERMISSION }) {
						$activeUser = true;
					}
				} else {
				...
				if ($activeUser) {
					...
					if ($result->isValid()) {
						$data = $authAdapter->getResultRowObject(null, USER_PASSWORD); //nnão armazena o password na sessnão
						$auth->getStorage()->write($data); //escreve na sessnão
	...
	/**
	 * Configura e recupera o componente de autenticação;
	 * 
	 */
	...
	if ($identity != null) {
		if ($fieldname == AUTH_ID_GROUP_USER) {
			if (!is_null($identity-> {
				...
				$where = $objGroup->getAdapter()->quoteInto(AUTH_ID_GROUP_USER . ' = ?', $identity-> {
					$fieldname });
...
private static function getGroupResource($groupId, $resourceId) {
	try {
		//verifica a relação resourceid/groupid, qual a configuração de permissnão
...
public static function getGroupResources() {
	try {
		...
		foreach ($res as $r) {
			$type = new Resource();
			$arrReturn[] = $type->find($r-> {
				AGR_RESOURCE_ID })->current();
		}
...
public static function getRoleResources() {
	if (isset ($_SESSION["roleResources"]) && $_SESSION["roleResources"] != null) {
		return unserialize($_SESSION["roleResources"]);
	}
	...
	try {
		...
		== $config->user->role->coordinator)) {
			if ($identity-> {
				...
				$_SESSION["roleResources"] = serialize($arrReturn);
				return $arrReturn;
			}
		} else {
			$type = new RoleResource();
			$where = $type->getAdapter()->quoteInto(ARR_ID_ROLE . " = " . $identity-> {
				AUTH_ID_ROLE_USER });
			$res = $type->fetchAll($where);
			$arrReturn = array ();
			foreach ($res as $r) {
...
public static function getResources() {
	//echo "adm"; print_r(AuthBusiness::isAdministrator());
	//echo "mng"; print_r(AuthBusiness::isManager());
	//echo "coo"; print_r(AuthBusiness::isCoordinator());
	if (AuthBusiness :: isAdministrator() || AuthBusiness :: isManager() || AuthBusiness :: isCoordinator()) {
		//nnão possuem grupos - buscar permissões no role-resources
...
/**
 * Verifica se o usuário possui permissnão para alterar a confidencialidade
 * do atendimento;
 * 
...
public static function allowChangeDefaultConfidentiality($resourceName) {
	//usuário Adm, gerente e coordenador nnão possuem essa limitação
	if (AuthBusiness :: isAdministrator() || AuthBusiness :: isManager() || AuthBusiness :: isCoordinator())
		return TRUE;
	...
	try {
		...
		if ($groupId != "" && $groupId != null) {
			...
			if ($perm-> {
				AGR_CHANGE_CONFIDENTIALITY }
			== 1)
			return TRUE;
		else
			return FALSE;
	}
	Logger :: loggerError("Database truncado. Usuário com perfil operador/técnico nnão possui grupo associado.");
	return FALSE;
} catch (Zend_Exception $e) {
...
/**
 * Retorna qual a confidencialidade padrnão para o atendimento;
 * 
 * throws UserNotLoggedException, InvalidResourceForThisOperation
 * 
 */
...
/**
 * Verifica se o usuário atual tem apenas permissnão somente-leitura ao resource atual;
 * 
 * throws UserNotLoggedException, InvalidResourceForThisOperation
 * 
 */
...
public static function getProfiles() {
	...
	try {
		...
		if (count($rows) > 0) {
			foreach ($rows as $k => $v) {
				if ($v->findParentRow(CLS_AUTH_PROFILE)-> {
					AUTH_STATUS }
				!= Constants :: DISABLE) {
...
/**
* Verifica se o usuário tem permissnão de acesso para o nome do controller

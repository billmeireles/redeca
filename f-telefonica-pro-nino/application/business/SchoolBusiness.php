...
class SchoolBusiness extends BasicBusiness
{
	...
	// Ciclo 2 - RF014 construir pagina html do coordenador com resultados do filtro escola aplicado
	public static function findForId($id) {
		$db = Zend_Db_Table :: getDefaultAdapter();

		$select = $db->select()
					 ->distinct()
					 ->from(array('es' => 'edu_school'), array('es.id_school', 'es.name'))
					 ->where('es.id_school = ?', $id);

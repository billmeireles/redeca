 ...
 * Author                                           Date                               History 
 * -----------------------------------------        ------------                       ------------------ 
 * Saulo Esteves Rodrigues  - S2it		   			31/01/2008	                       Create file 
 * 
 */
		...
		foreach($person as $prs)
		{
			...
			{		
				...
				else
				{
	 			...
	 			if($address != null && $address->findParentRow(CLS_ADDRESSTYPE)->{ADT_DESCRIPTION} && strlen($address->findParentRow(CLS_ADDRESSTYPE)->{ADT_DESCRIPTION}) > 0)
	 			{
				...
				if($address != null && $address->{ADR_ID_ADDRESS} && strlen($address->{ADR_ID_ADDRESS}) > 0)
				{
				...
				if($address != null && $address->{ADR_ADDRESS} && strlen($address->{ADR_ADDRESS}) > 0)
				{
				...
				if($address != null && $address->{ADR_ZIP_CODE} && strlen($address->{ADR_ZIP_CODE}) > 0)
				{
